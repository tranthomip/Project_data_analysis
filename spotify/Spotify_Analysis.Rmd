...
library(corrplot)
corrplot(cor(new_song1), method = 'square')
...
**Effect of acousticness and energy on popularity**

```{r}
ggplot(new_song, aes(energy, popularity)) + geom_point(color = "darkblue") + geom_smooth(method = 'lm')
ggplot(new_song, aes(acousticness, popularity)) + geom_point(color = "darkgreen") + geom_smooth(method = 'lm')
```

No relationship between energy, acousticness with popularity.
...
```{r}
setwd("C:/Users/Laptop88/Downloads")
```
...
ggplot(mean_song) + geom_line(aes(year, danceability, color = "danceability")) + geom_line(aes(year, energy, color = "energy")) + geom_line(aes(year, acousticness, color = "acousticness")) + geom_line(aes(year 
...
EFFECT OF ENERGY AND ACOUSTICNESS ON POPULARITY
